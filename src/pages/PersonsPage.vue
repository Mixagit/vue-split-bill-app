<template>
	<div class="container">
		...
		>
			...
			<p>Вы можете добавить людей, которые есть в вашем чеке по кнопке выше</p>
		</div>
	...
	methods: {
		...
		goToProductsPages() {
			...
			if (this.persons.length < 2) {
				this.isDisabledNextBtn = true
				this.nextButtonText = this.persons.length ? 'Добавьте еще кого-нибудь' : 'Но тут же никого нет!'
				setTimeout(() => {
					this.isDisabledNextBtn = false
					this.nextButtonText = 'Перейти к добавлению позиций в меню'
				}, 2000)
